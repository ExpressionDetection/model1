...
class MicroExpNet():
	def __init__(self, x, y=None, teacherLogits=None, lr=1e-04, nClasses=8, imgXdim=84, imgYdim=84, batchSize=64, keepProb=0.0, temperature=8, lambda_=0.5):
		self.x = x
		self.w = {}
		self.b = {}
		self.y = y 
	...
	def initParameters(self):
		...
		'wc1': tf.compat.v1.get_variable('wc1', [8, 8, 1, 16], initializer=tf.contrib.layers.xavier_initializer_conv2d()),
		# 4x4 conv, 16 inputs, 32 outputs
		'wc2': tf.compat.v1.get_variable('wc2', [4, 4, 16, 32], initializer=tf.contrib.layers.xavier_initializer_conv2d()),
		# fully connected, 1152 inputs, 48 outputs
		'wfc': tf.compat.v1.get_variable('wfc', [32*6*6, self.fcOutSize], initializer=tf.contrib.layers.xavier_initializer()),
		# 48 inputs, 8 outputs (class prediction)
		'out': tf.compat.v1.get_variable('wo', [self.fcOutSize, self.nClasses], initializer=tf.contrib.layers.xavier_initializer())
		}
		self.b = {
